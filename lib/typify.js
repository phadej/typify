  ...
  function compileCheckableType(environment, context, parsed) {
    if (parsed.type === "var") {
      ...
      if (environment.has(parsed.name)) {
        ...
        var args = parsed.args.map(compileCheckableType.bind(undefined, environment, context));
        return function (varCheck, arg) {
      ...
      var c = compileCheckableType(environment, context, parsed.term);
      return function (varCheck, arg) {
        return arg === undefined || c(varCheck, arg);
      };
    } else if (parsed.type === "alt") {
      var cs = parsed.options.map(compileCheckableType.bind(undefined, environment, context));
      return function (varCheck, arg) {
  ...
  function record(environment, name, definition) {
    ...
    for (var k in definition) {
      if (has(definition, k)) {
        ...
        checks[k] = compileCheckableType(environment, {}, parsed);
      }
    }
